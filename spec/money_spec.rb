...
describe Money do
  ...
  specify "#eql? returns true if and only if their amount and currency are equal" do
    ...
    Money.new(1_00, "USD").eql?(Money.new(2_00, "USD")).should be_false
    Money.new(1_00, "USD").eql?(Money.new(99_00, "EUR")).should be_false
  end

  specify "#eql? can be used to compare with a String money value" do
    Money.new(1_00, "USD").eql?("1.00").should be_true
    Money.new(1_00, "USD").eql?("2.00").should be_false
    Money.new(1_00, "GBP").eql?("1.00").should be_false
  end

  specify "#eql? can be used to compare with a Numeric money value" do
    Money.new(1_00, "USD").eql?(1).should be_true
    Money.new(1_57, "USD").eql?(1.57).should be_true
    Money.new(1_00, "USD").eql?(2).should be_false
    Money.new(1_00, "GBP").eql?(1).should be_false
  end
  ...
  specify "#eql? can be used to compare with an object that responds to #to_money" do
    ...
    Money.new(1_00, "USD").eql?(klass.new(Money.new(1_00, "USD"))).should be_true
    Money.new(2_50, "USD").eql?(klass.new(Money.new(2_50, "USD"))).should be_true
    Money.new(2_50, "USD").eql?(klass.new(Money.new(3_00, "USD"))).should be_false
    Money.new(1_00, "GBP").eql?(klass.new(Money.new(1_00, "USD"))).should be_false
  end

  specify "#eql? returns false if used to compare with an object that doesn't respond to #to_money" do
    Money.new(1_00, "USD").eql?(Object.new).should be_false
    Money.new(1_00, "USD").eql?(Class).should be_false
    Money.new(1_00, "USD").eql?(Kernel).should be_false
    Money.new(1_00, "USD").eql?(/foo/).should be_false
    Money.new(1_00, "USD").eql?(nil).should be_false
  end
